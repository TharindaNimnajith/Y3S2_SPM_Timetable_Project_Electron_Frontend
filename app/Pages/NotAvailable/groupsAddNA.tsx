...
const GroupsAddNA: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setGroup1List(responseData.groups);
   ...
   if(workingTimePerDay1.minutes <= 9 && workingTimePerDay1.minutes!== '00'){
    ...
    if ((id1.trim() === '') && (day.trim() === '') && (stime.trim() === '00:00') &&  (etime.trim() === '00:00')){
      ...
      try {
        ...
        const response = await fetch(`${proxy}/groups/addNotAvailable/` + id1, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  //   setUnavailability({day:day, startTime: stime , endTime : etime});
  // }
  const handleGroup1 = (e: React.ChangeEvent<HTMLInputElement>) => {
    setError(false)
  ...
  const getGroup1 = async (id: string) => {
    setLoading(true)
    try {
      const response = await fetch(`${proxy}/groups/getGroups/` + id, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
   ...
   <div
    ...
    >
      ...
      >
        ...
        <Modal.Body>This group already has a not available time. If you want you can replace that or keep it as it is.</Modal.Body>
        <Modal.Footer>
                ...
                <Form.Group controlId="formBasicEmail">
                  ...
                  >
                    ...
                    {group1List?.map((group, index) => (
                      <option value={group._id}>{group.groupId}</option>
                    ))}
                  </Form.Control>
