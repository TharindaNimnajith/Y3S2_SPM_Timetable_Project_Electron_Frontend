...
const SubGroupsAddNA: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setGroupsObject(responseData.subGroups);
      setGroup1List(responseData.subGroups);

      //dispatch(setConsecutiveSessions(responseData));
      console.log(responseData.subGroups);
  ...
  const handleSubmit = async () => {
   ...
   if(workingTimePerDay1.minutes <= 9 && workingTimePerDay1.minutes!== '00'){
    ...
    if ((id1.trim() === '') && (day.trim() === '') && (stime.trim() === '00:00') &&  (etime.trim() === '00:00')){
      ...
      if (id1.trim() === '') {
        errors_ = 'Please select a sub group .'
        setError(true)
        setLoading(false)
      } else if (day.trim() === '') {
      ...
      try {
        ...
        const response = await fetch(`${proxy}/subGroups/addNotAvailable/` + id1, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  const getGroup1 = async (id: string) => {
    setLoading(true)
    try {
      const response = await fetch(`${proxy}/subGroups/getSubGroups/` + id, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
                ...
                <Form.Group controlId="formBasicEmail">
                  ...
                  >
                    ...
                    {group1List?.map((group, index) => (
                      <option value={group._id}>{group.subGroupId}</option>
                    ))}
                  </Form.Control>
